...
def prob_appear(E, fermi, T):
    ...
    for i in range(len(E)):
        if(E[i] < fermi):
            ...
            ubound = 1 - np.exp(-(E[i] - fermi) / (kB * T)) / (1 + np.exp(-(E[i] - fermi) / (kB * T)))
            fout.append(ubound)
            #print('n')
...
def line_appear(elem_name, vel):
    ...
    #Create a samle velocity domain and find the nearest index to passed velocity
    ...
    elements = {
        ...
        'Ionization Energy (eV)' : [13.6, 11.26, 13.6, 7.64, 5.99, 8.15, 6.11, 7.9],
        'Ionization Efficiency (eV)' : [13.6, 11.26, 13.6, 7.64, 5.99, 8.15, 6.11, 7.9],
        '10% Velocity (km/s)' : [6.6, 6.9, 12.2, 4.1, 4.1, 6.2, 3.9, 6.7],
        '90% Velocity (km/s)' : [12.0, 12.7, 17.0, 7.4, 7.8, 11.6, 5.5, 10.6]
    }
    
    #Is the provided elemntin the database?
    if(elem_name in elements['Element Names']):
        
        elem_index = elements['Element Names'].index(elem_name)
        #print(elem_index)
        
        #Fetch velocities for 10, 50 and 90 percent probabilities from database
        ...
        #Fit coefficients of arctan functions by passing in the function, the x data, the y data, and finally an initial 
        #estimate for the parameters, I use [(1/3), 10, 1.5]
        parameters, covariance = curve_fit(atan_approx, vel_vals, prob_vals, p0 = [(1/3), 10, 1.5])
    
        #Estimate probability using above PDF and given velocity
        g = atan_approx(vel_domain, parameters[0], parameters[1], parameters[2])
        ...
        if(rand <= .5 + g[v_indx]):
            ...
            #If this is true the line appears, scale the amplitude by the TOF-SIMS-like ionization efficiency
            #SIMS-like efficiencies given in Table 2 of Hornung and Kissel (1994)
...
if __name__ == "__main__": 
    ...
    vel_vals = np.array([elements['10% Velocity (km/s)'][3], elements['50% Velocity (km/s)'][3], elements['90% Velocity (km/s)'][3]])
    prob_vals = np.array([.1, .5, .9])
    ...
    ax.set_title("Hydrogen line of Best Fit", fontweight = 'bold')
